...
static WNDPROC lpMyWndProc_ = NULL;                     // 独自拡張をしたウィンドウプロシージャ
static WNDPROC lpOriginalWndProc_ = NULL;               // 元に設定されていたウィンドウプロシージャ（あれば）
static BOOL bIsWndProcActive_ = FALSE;                  // 独自ウィンドウプロシージャを機能させるか。破棄の最中にはFALSEにする
...
LRESULT CALLBACK customWindowProcedure(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    // 独自ウィンドウプロシージャが破棄中の場合、lpMyWndProc_ も NULL になる可能性があるためデフォルト処理のみで終了
    if (bIsWndProcActive_ == FALSE) {
...
void destroyCustomWindowProcedure() {
    ...
    // Disable processing in the custom window procedure
    bIsWndProcActive_ = FALSE;
    ...
    // Clear the custom window procedure pointer
    lpMyWndProc_ = NULL;
}
