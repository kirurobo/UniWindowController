...
void UNIWINC_API SetTopmost(const BOOL bTopmost) {
	if (hTargetWnd_) {
		SetWindowPos(
			...
			0, 0, 0, 0,
			SWP_NOSIZE | SWP_NOMOVE | SWP_FRAMECHANGED | SWP_NOOWNERZORDER | SWP_NOACTIVATE | SWP_ASYNCWINDOWPOS
		);
	}
...
BOOL UNIWINC_API SetPosition(const float x, const float y) {
	...
	return SetWindowPos(
		...
		(int)x, (int)newY,
		0, 0,
		SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_NOSIZE | SWP_NOZORDER | SWP_FRAMECHANGED | SWP_ASYNCWINDOWPOS
		);
}
